...
export class RoomService {
   ...
   */
  async getAllRooms(userRole: string, userId?: string): Promise<RoomWithDetails[]> {
   ...
   */
  async getRoomById(id: number, userRole: string, userId?: string): Promise<RoomWithDetails | null> {
    // Check if user has access to this room
    if (userRole === 'USER' && userId) {
      const hasAccess = await this.checkUserRoomAccess(userId, id);
      if (!hasAccess) {
   ...
   */
  async getRoomByNumber(roomNumber: number): Promise<Room | null> {
  ...
  async createRoom(data: CreateRoomData): Promise<Room> {
    ...
    const existingRoom = await this.getRoomByNumber(data.roomNumber);
    if (existingRoom) {
   ...
   * Get rooms by floor
   */
  async getRoomsByFloor(floor: number, userRole: string, userId?: string): Promise<RoomWithDetails[]> {
   ...
   */
  async checkUserRoomAccess(userId: string, roomId: number): Promise<boolean> {
   ...
   */
  async getRoomOccupancyStats(): Promise<{
    totalRooms: number;
