...
export class UserService {
   ...
   */
  static async syncWithAuth0(auth0Profile: any) {
   ...
   */
  static async assignUserToRooms(userId: string, roomIds: number[]) {
    try {
      // Verify user exists
   ...
   */
  static async getUserRoomAssignments(userId: string) {
    try {
   ...
   */
  static async removeUserRoomAssignment(userId: string, roomId: number) {
    try {
   ...
   */
  static async addUserRoomAssignment(userId: string, roomId: number) {
    try {
      // Verify user exists
      const user = await prisma.user.findUnique({
        where: { id: userId }
   ...
   */
  static async hasRoomAccess(userId: string, roomId: number): Promise<boolean> {
    try {
      const user = await prisma.user.findUnique({
        where: { id: userId }
      });
